...
7. Если фраза содержит анафору к уже выбранному врачу, не давать ложному catalog hit из обычной речи заменить remembered doctor.
...
- `Хорошо! А можешь найти расписание его работы?` не превращает `Хорошо` в фамилию врача;
...
6. Проверить boundary между FT adapter и legacy `doctors_schedule_week`: `branch_name` не должен интерпретироваться как город.

Критерий готовности:

- выходные не показывают будни;
- branch follow-up не уходит в handoff;
- `branch_name=Ленина` остается фильтром расписания, а `city=Оренбург` по-прежнему дает `city_not_supported`.
