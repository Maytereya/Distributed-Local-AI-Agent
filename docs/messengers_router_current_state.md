...
└── services/               — доменный слой (бывший монолит services.py 7288 стр., распилен)
    ...
    ├── appointments.py     — appointment_help (⚠️ сейчас недостижим — см. чекап)
    └── news.py             — новости
```
...
- ✅ Гейт `pytest` снова **зелёный** (`839 passed, 0 failed`, ruff clean) — красный гейт закрыт.
- ✅ **H1** (грундер «профиль 1»→«профиль 2») — исправлено (`a50b11a`).
- ✅ **H2** (протухший `no_free_slots` выбрасывал реальное расписание со слотами) — исправлено (`4e3469d`).
- ✅ **H3** (`@lru_cache` кешировал «врач не найден» до перезапуска процесса) — исправлено (`1c609c7`).
- 🟡 **Ещё открыто:** multi-word несамарский город проскакивает гео-гейт; слишком широкие regex модификаторов цены (`ген\w*`/`экспресс\w*`/`дет\w*`); прочие MEDIUM и мёртвый код — см. чекап.
