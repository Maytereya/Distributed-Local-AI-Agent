...
**⚠️ OWNER-CHECKPOINTS (подтвердить у владельца, НЕ блокируют написание кода — блокируют коммит затронутой задачи):**
- **OC-1 (Task 4):** для РЕЗУЛЬТАТОВ при 5xx/таймаут — заменить авто-эскалацию на оператора (`service_error_results`) на честный `tech_unavailable` БЕЗ авто-handoff? (мягче, без спама оператора; пользователь сам пишет «оператор»). Сейчас 5xx→оператор.
- **OC-2 (Task 8):** числа realtime fail-fast профиля: `total` ретраев (0 или 1) и read-timeout (ориентир 5-8с).
- **OC-3:** финальная формулировка `tech_unavailable_text` (как реворд BUG-05 — утверждается владельцем).
