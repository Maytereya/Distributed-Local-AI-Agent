...
- **НЕ фиксилось детерминированно (по решению владельца):** прямое извлечение услуги из turn A («Нет, лучше подскажи стоимость ОАК») — это работа LLM/классификатора (интент LLM роутит верно даже с разговорным зачином: TEST_RESULT/налоговая/PRICE; отваливается лишь извлечение `service_name`). Стоп-слова в прайс-матчере отвергнуты как brittle и интент-связанные. Рычаг — промпт `classifier_patient.txt` (правка по решению владельца, валидация через remote eval). После фикса петля закрыта; turn A даёт максимум один clarify, а не цикл.
