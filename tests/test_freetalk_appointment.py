...
def test_schedule_to_appointment_flow_uses_slot_and_finishes_with_handoff():
    ...
    # Compute the expected date dynamically for the actual test input:
    # "16.04, 09:00" should resolve to the next upcoming April 16.
    _today = datetime.date.today()
    _apr16 = datetime.date(_today.year, 4, 16)
    _expected_date = _apr16.isoformat() if _apr16 >= _today else _apr16.replace(year=_today.year + 1).isoformat()
