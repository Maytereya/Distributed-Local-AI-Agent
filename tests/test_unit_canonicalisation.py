...
@pytest.mark.parametrize(
    ...
    [
        ...
        # «Врач-хирург, флеболог» — хирург ОБЫЧНЫЙ (профиль не уточнён), поэтому
        # отвечает и на «хирург», и на «флеболог».
        ("Врач-хирург, флеболог", "хирург"),
        ("Врач-хирург, флеболог", "флеболог"),
        # Средний медперсонал по физиотерапии — как «Медицинская сестра по массажу».
        ("Медицинская сестра по физиотерапии", "физиотерапевт"),
        # «Врач-сердечно-сосудистый хирург, флеболог[, лимфолог]» отвечает на
        # КАЖДУЮ перечисленную роль, но НЕ на базовый «хирург» (см. блок 1).
