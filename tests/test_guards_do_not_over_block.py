...
* **И-Б (гард мульти-расчёта).** Если корзина распозналась полностью
  (≥2 позиции, нераспознанных нет), расчёт обязан быть выдан. Закрыт 16.09
  переходом гарда на ОСТАТОК: судить пациентское слово, которое резолвер уже
  поглотил, нечем и незачем — оно удовлетворено по определению. Остаток
  («ОМС» после разбора «приём терапевта по ОМС») гард судит по-прежнему,
  поэтому П2 не регрессирует. См. BUG-2026-09-07-GUARD-OVER-BLOCK.
"""
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # «Липидограмма», но самого слова в каталоге нет.
...
def test_multi_price_survives_patient_synonyms(cart: str) -> None:
    """И-Б для пациентской лексики.

    Был `xfail(strict=True)` с 07.09: гард судил реплику ЦЕЛИКОМ и объявлял
    пациентское слово неудовлетворимым уточнением на основании словаря каталога.
    Закрыт переходом гарда на остаток (решение владельца 16.09), см.
    BUG-2026-09-07-GUARD-OVER-BLOCK.
    """
    items, unrecognized = _resolve_multi_price_items(cart, ROWS)
    assert len(items) >= 2 and not unrecognized, f"предусловие не выполнено: {cart!r}"
    assert _build_multi_price_payload(cart, ROWS) is not None
