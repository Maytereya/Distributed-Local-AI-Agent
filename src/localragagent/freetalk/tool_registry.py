...
_MEDICAL_TOPIC_RE = _compile(
    r"\b("
    r"врач|доктор|специалист|прием|консультац|расписан|график|услуг|процедур|"
    r"анализ|тест|подготовк|цена|стоим|прайс|адрес|филиал|результат"
    r")\w*\b"
)
...
_DOCTOR_RE = _compile(r"\b(врач|доктор|к\s+[а-яё\-]{3,})\b")
...
_WEB_SIGNAL_RE = _compile(
    ...
    r"today|latest|breaking\s+news"
    r")\b"
)
