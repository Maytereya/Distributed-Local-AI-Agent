...
# --- слова речевого ДЕЙСТВИЯ, а не услуги ------------------------------
# Эти слова УЖЕ считались обвязкой другими слоями — `_SPEECH_WRAPPER`
# (слой синонимов МИС) и `_MULTI_PRICE_NOISE_WORDS` (разбор корзины), — но
# прайсовый токенайзер о них не знал. Одно понятие жило в трёх списках,
# которые разошлись, и расхождение стоило ответов: лишний токен раздувает
# `token_count`, а `_is_price_match_strong` при двух токенах требует, чтобы
# совпали ОБА. Отсюда «ферритин» → Ферритин, а «прайс ферритин»,
# «записаться на ферритин», «посчитайте ферритин» → None.
#
# Состав НЕ придуман: взяты слова из тех двух списков, которых нет здесь, и
# отброшены все, что встречаются в названиях услуг каталога — «анализ» (49
# строк), «тест» (55), «после» (29), «перед» (9), «себя» (2) остались
# нетронутыми, иначе сломался бы матч «Общий анализ крови».
# Свип пересчитывается тестом `test_speech_act_stopwords_absent_from_catalog`.
# См. BUG-2026-09-17-FILLER-LISTS-DISAGREE.
_PRICE_SPEECH_ACT_STOPWORDS = frozenset({
    "анализы",
    "вас",
    "вечер",
    "возле",
...
_VITAMIN_CODE_MAP = {
    "а": "a",
    "a": "a",
    # «в» — ВИЗУАЛЬНЫЙ гомоглиф латинской «B», «б» — ФОНЕТИЧЕСКАЯ запись того же
    # кода. Пациент пишет и так, и так («витамин б6» / «витамин в6»), а карта
    # изначально покрывала только гомоглифы: BUG-2026-09-09-CODE-FAMILY, из-за
    # чего ВСЯ группа B (б1, б2, б3, б5, б7, б12) садилась на «Витамин B12».
    "в": "b",
    "б": "b",
    "b": "b",
    "с": "c",
...
# «б» — не гомоглиф: на латинскую «B» она не похожа, пациент набирает её
# ФОНЕТИЧЕСКИ. Но в lab-кодах он делает это ровно так же часто, как подставляет
# похожую «в», и результат для него один и тот же код. Карта покрывает обе оси —
# сходство начертания и сходство звучания (BUG-2026-09-09-CODE-FAMILY: «витамин
# б12» не находился вовсе, потому что токен «б12» не встречается в каталоге и
# гард уточнений гасил матч).
_PRICE_CYR_TO_LAT: dict[str, str] = {
    "а": "a", "е": "e", "о": "o", "р": "p", "с": "c",
    "х": "x", "у": "y", "к": "k", "в": "b", "б": "b", "н": "h",
    "м": "m", "т": "t",
}
...
def _match_drops_unsatisfiable_qualifier(
    ...
    for token in tokens:
        ...
        if token in vocab:
            continue
        # Тот же токен в ДРУГОМ алфавите — не другое слово. Токенайзер уже
        # считает «б12» и «b12» одним lab-кодом и кладёт оба в запрос; гард
        # обязан судить так же, иначе кириллическая запись выглядит для него
        # «уточнением, которого нет в прайсе». Правило узкое: фолдинг работает
        # ТОЛЬКО на токенах с цифрой (признак кода), поэтому «омс», «онлайн»,
        # «холестерин» под него не попадают и остаются неудовлетворёнными —
        # исходный П2 и открытый over-block этим не затрагиваются.
        folded = _fold_homoglyph_token(token)
        if folded and (folded in vocab or _token_covered_by(folded, canon_tokens)):
            continue
        return True
    return False
