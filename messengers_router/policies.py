...
# Подмножество TEST_ASSIST_PATTERNS, описывающее ЦЕЛЬ подбора («какие анализы
# сдать…», «подобрать анализы», «комплекс анализов», «чекап», «скрининг»), а не
# НАЗВАНИЕ услуги. Ровно для этих формулировок и существует воронка «Для какой
# цели хотите подобрать анализы?».
#
# Зачем отдельный список (П1): catalog-матчер приземляет и такие реплики на
# случайную строку прайса по одному общему токену («какие анализы сдать для
# ПРОФИЛАКТИКИ» → «Здоровая молодость: курс для профилактики и восстановления»).
# Переклеивать такую реплику в PRICE нельзя — пациент получит цену чужой услуги
# вместо вопроса о цели. Список НЕ про названия услуг (не стоп-лист): это
# грамматические формы вопроса о цели.
TEST_ASSIST_GOAL_PATTERNS = [
    r"\bподобрат(ь|ь)\s+анализ",
    r"\bкакие анализ(ы)?\s+[сз]дат(ь|ь)\b",
    r"\bчто\s+[сз]дат(ь|ь)\b.*\bанализ",
    r"\bчекап\b",
...
WORK_HOURS_PATTERNS = [
    ...
    # Конкретный врач отсекается guard'ом _DOCTOR_MENTION_RE в detect_clinic_hours_intent
    # («расписание работы врача Иванова» остаётся DOCTOR_SCHEDULE).
    r"\bрасписани\w*.*\bработ\w*",
]
...
NEWS_PATTERNS = [
    ...
    r"\bновост\w*",
    # Синонимы без слова «акция» (v1.2): однозначные promo-маркеры. «выгодно/
    # дешевле» НЕ добавляем — это чаще price/сравнение («где дешевле ОАК») и
    # ловится LLM-классификатором (промпт: NEWS-синонимы), не хрупким правилом.
    r"\bпромокод\w*",
    r"\bбонус\w*",
    r"\bакционн\w*",
]
...
HANDOFF_REASON_MATRIX: dict[str, str] = {
    ...
    # Результаты анализов: тех-сбои идут через честный tech_unavailable (resilience Фаза 1),
    # а deep-link на результат больше не формируется (BUG-2026-06-08-01) — отдельные
    # service_error_results / service_error_result_link стали недостижимы и удалены.
...
def detect_test_assist_goal_phrasing(text: str) -> bool:
    """True, если реплика формулирует ЦЕЛЬ подбора анализов, а не название услуги.

    Используется как гард переклейки TEST_ASSIST → PRICE (П1): такую реплику
    catalog-матчер тоже приземляет на строку прайса (по одному общему токену),
    но спрашивают в ней цель, а не цену конкретной услуги.

    :param text: исходный текст запроса пользователя
    :return: True для «какие анализы сдать…», «подобрать анализы», «чекап», …
    """

    return _matches_any(text, _TEST_ASSIST_GOAL_RE)
...
WEBSITE_HELP_TEXT = (
    "Я виртуальный помощник клиники и помогаю с записью, услугами, ценами, анализами "
    "и адресами филиалов. С работой сайта (личный кабинет, корзина, онлайн-оплата) "
    "помочь не смогу — обратитесь, пожалуйста, в техподдержку клиники."
)

# Навигация по САЙТУ (онлайн-корзина / личный кабинет / «сайт не работает») — НЕ
# физический адрес и не услуга клиники. Без этого «Где находится корзина на сайте?»
# срабатывало в ADDRESS-правиле и бот выдавал филиалы (BUG-C). Специфично к сайту,
# чтобы НЕ задеть будущую bot-side «корзину анализов», «добавь в корзину» и запись
# «на сайте». ReDoS-safe: квантификаторы ограничены (`{0,20}`), без вложенных.
WEBSITE_HELP_PATTERNS = [
    r"корзин\w*\s+(?:на\s+|в\s+)?(?:сайт\w*|сайте|онлайн)",
    r"(?:сайт\w*|сайте)\b[^.?!]{0,20}\bкорзин",
    r"\bличн\w+\s+кабинет\w*",
    r"как\s+пользоват\w+\s+(?:сайт\w*|личн\w+\s+кабинет)",
    r"\bсайт\w*\s+не\s+работает",
]
_WEBSITE_HELP_RE = _compile_patterns(WEBSITE_HELP_PATTERNS)


def detect_website_help_intent(text: str) -> bool:
    """Вопрос про навигацию по веб-сайту (корзина/личный кабинет на сайте, «сайт
    не работает») — не физический адрес и не услуга клиники → честный website-help.

    Специфично к сайту: НЕ ловит bot-side «корзину анализов», «добавь в корзину» и
    запись «на сайте» — только явную навигацию по веб-сайту/онлайн-кабинету.

    :param text: текст пользователя
    :return: True, если это вопрос про работу веб-сайта/онлайн-кабинета
    """
...
SICK_LEAVE_HANDOFF_TEXT = "Вопрос про больничный лист уточню у оператора — соединяю."

# П9 faithfulness: 25.06 бот галлюцинировал «клиника НЕ оформляет больничные»
# (на деле оформляет — но факты владельцем ещё не подтверждены) → до
# подтверждения БЛ-интент идёт на оператора, не факт-ответ. Грамматический
# анти-FP: sick-leave — маскулинные формы («больничный лист»), а улица
# Больничная (филиал на ул. Больничная 44Г в Красном Яру) — феминные
# («Больничная/Больничной/Больничную») → в паттернах ТОЛЬКО маскулинные/plural
# окончания + «лист(ок) нетрудоспособности», феминные не матчатся by design.
# ReDoS-safe: без вложенных квантификаторов, повторы ограничены.
SICK_LEAVE_PATTERNS = [
    r"\bбольничн(?:ый|ого|ому|ым|ом|ые|ых|ыми)\b",
    r"\bбольничн\w{1,4}\s+лист\w{0,4}\b",
    r"\bлист\w{0,4}\s+нетрудоспособ\w{1,8}\b",
    r"\bнетрудоспособн\w{1,8}\b",
]
_SICK_LEAVE_RE = _compile_patterns(SICK_LEAVE_PATTERNS)


def detect_sick_leave_intent(text: str) -> bool:
    """Вопрос про больничный лист / лист(ок) нетрудоспособности.

    Пока факты об оформлении БЛ не подтверждены владельцем, интент ведёт на
    оператора (П9): честный handoff вместо галлюцинации «не оформляем».
    Феминные формы («Больничная улица», «на Больничной 44Г» — адрес филиала
    в Красном Яру) и «лист назначений»/«прайс-лист» НЕ считаются интентом.

    :param text: текст пользователя
    :return: True, если это вопрос про больничный лист
    """

    raw = str(text or "").strip()
    if not raw:
        return False
    return _matches_any(raw, _SICK_LEAVE_RE)
...
# ---------------------------------------------------------------------------
# Homoglyph folding for Cyrillic→Latin (для сравнения lab-кодов).
#
# Применяется ТОЛЬКО как ДОПОЛНИТЕЛЬНЫЙ кандидат в матчере (не заменяет
# оригинал) — чтобы не ломать русскоязычный поиск. «Сахар»/«рак» в обычных
# запросах остаются нетронутыми; фолдинг применяется лишь к токенам,
# которые содержат цифру (признак лаб-кода), или как доп-ключ при поиске.
# ---------------------------------------------------------------------------
...
def fold_cyrillic_homoglyphs(text: str) -> str:
    """Заменяет визуально-идентичные кириллические символы на латинские.

    Используется ТОЛЬКО как дополнительный ключ для сравнения строк
    (например, «Са-125» → «Ca-125» для матча с «CA - 125»).
    НЕ применяется к user-facing тексту и не изменяет каталожные данные.
    Применять только к коротким токенам, содержащим цифру, или явно
    в контексте лаб-кода — иначе русские слова могут неверно матчиться.

    :param text: исходная строка (может содержать кириллицу и латиницу)
    :return: строка с заменёнными гомоглифами
    """
    return "".join(_CYR_TO_LAT.get(ch, ch) for ch in (text or ""))


# Паттерн для детектирования bare-name лаб-кодов в запросе пользователя.
# Структура: короткий БУКВЕННЫЙ ПРЕФИКС маркера → сразу ЦИФРА → необяз. хвосты.
# Примеры: «CA-125», «Са-125», «CA 19-9», «СА 19-9», «HbA1c», «CA 15-3».
#
# Буквы префикса ограничены латиницей + кириллическими ГОМОГЛИФАМИ (визуально
# идентичными латинским: а е о р с х у к в н м т и заглавные). Произвольную
# кириллицу НЕ берём — иначе «адрес + номер» («Гагарина 64», «Ленина 5»,
# «Победы 83», «Кирова 223») ложно опознаётся как лаб-код (BUG-2026-06-04-03).
# Структура «префикс→цифра» также отсекает «<слово> <число>» (после префикса
# идёт буква, а не цифра → нет матча): «Сухова 12», «кабинет 5» и т.п.
_LAB_CODE_RE = re.compile(
    r"^\s*"
    r"[A-Za-zаеорсхуквнмтАЕОРСХУКВНМТ]{1,5}"      # буквенный префикс (латиница + кир. гомоглифы)
    r"[\s\-/]*\d{1,3}"                             # сразу за префиксом — число
    r"(?:[\s\-/]*[0-9A-Za-zаеорсхуквнмт]{1,3})*"   # необяз. хвосты: «19-9», «1c», «15-3»
    r"\s*$",
    re.I,
)

# Дополнительный паттерн: известные лаб-коды без цифр (PSA, CEA, ПСА, РЭА, ТТГ и т.п.)
# уже покрыты TEST_ASSIST_PATTERNS через _PRICE_SHORT_TOKEN_WHITELIST или TEST_ASSIST_PATTERNS[last].
# Здесь добавляем только «alphanumeric+digit» класс.


def detect_specific_lab_code_intent(text: str) -> bool:
    """True, если текст — bare-name лаб-кода (буквы + цифра), вероятно конкретный анализ.

    Детектирует запросы вида «Са-125», «CA-125», «CA 19-9», «HbA1c», «CA 15-3»,
    «PSA 19-9» и т.п. — буквенно-цифровые коды, которые пользователь вводит
    без дополнительного контекста. Такие запросы должны уходить в TEST_ASSIST
    с заполненным test_name, а не в LLM-generic.

    Намеренно НЕ матчит обычные русские слова: «сахар», «рак», «тироксин» и т.п.

    :param text: исходный текст пользователя
    :return: True, если похоже на конкретный лаб-код
    """
    raw = str(text or "").strip()
    if not raw or len(raw) > 30:
        return False
    return bool(_LAB_CODE_RE.match(raw))
...
_DISCLOSURE_SIGNATURE_SUBSTRINGS = (
    ...
    "gigachat",
    "yandexgpt",
    # Кириллические транслитерации вендоров / родовые маркеры само-представления
    # модели (аудит 2026-07-22: латинский денилист обходился кириллицей —
    # «Под капотом Мистраль», «Клод от Антропик» проходили к пациенту).
    # ВАЖНО: только подстроки, которых НЕТ в легитимных мед-названиях. Проверено
    # против живого прайса: «нейросет» (микроскопия с нейросетями) и «ламин»
    # (катехоЛАМИНы) — реальные услуги → НЕ добавлены (дали бы ложный дефлект).
    # Агрессивная нормализация пробелов/гомоглифов НЕ применяется по той же причине
    # (мед-аббревиатуры, латинские коды анализов). Денилист — бэкстоп; основная
    # защита — блок КОНФИДЕНЦИАЛЬНОСТЬ в промпте. Перевод/перефраз денилист не ловит.
    "мистрал",  # Мистрал/Мистраль
    "клод",  # Claude (0 совпадений в прайсе)
    "антропик",  # Anthropic (в прайсе «антропо-» — не коллизит)
    "джипити",  # GPT на слух
    "чатгпт",
    "чат-гпт",
    "гигачат",
    "яндекс gpt",
    "яндексгпт",
    "языковая модель",  # «большая языковая модель» — вне мед-лексикона
    "языковой модел",
