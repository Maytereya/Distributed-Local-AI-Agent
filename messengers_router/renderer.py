...
async def ollama_call(prompt: str, llm: str = LLMName.get(), think: bool = None, ) -> AsyncGenerator[str, Any]:
    ...
    async for _chunk in stream:
        _partial += _chunk["response"]
        yield _partial


def _ollama_result_to_text(result: Any) -> str:
    """
    Приводит результат ollama_call к тексту, который можно отдавать в поток.
    Ожидаемо result — dict (res.__dict__), но на всякий случай поддерживаем и другие типы.
    """
    if isinstance(result, dict):
        # Частые варианты ключей у LLM-ответов; порядок важен.
...
async def ollama_render_stream(prompt: str) -> AsyncGenerator[str, None]:
    # ЗАГЛУШКА: заменить на реальный streaming Ollama

    result = await ollama_call(prompt)

    yield _ollama_result_to_text(result)
...
async def render_stream(user_text: str, decision: RouteDecision, evidence: Evidence) -> AsyncGenerator[str, None]:
    ...
    async for chunk in ollama_render_stream(prompt):
