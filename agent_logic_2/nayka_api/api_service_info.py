...
def backfill_service_synonyms(path: Path | None = None) -> int:
    ...
    СТРАХОВКА, обычно не работает. История: 10.09 массовый `serviceInfoAll` отдавал
    `serviceSynonyms: null` у всех 8472 услуг, хотя поштучный метод по той же
    услуге возвращал значение — баг на стороне клиники. Обход поштучно был
    единственным способом получить словарь: ~1622 запроса, около 6 минут.

    15.09 клиника это починила, массовая выгрузка снова несёт синонимы (295 услуг,
    872 синонима). Поэтому при наличии хоть одного синонима в срезе обход
    пропускается — иначе это 1300+ бессмысленных запросов к чужому API каждое
    утро. Если выгрузка снова отдаст пусто, добор включится сам.

    Идёт ПОСЛЕ суточной выгрузки и вне пути пациента. Сбой на отдельной услуге не
    роняет прогон: значение просто остаётся пустым.
    ...
    # Клиника починила массовый метод 15.09.2026: `serviceInfoAll` снова отдаёт
    # `serviceSynonyms`. Если в срезе есть хоть один синоним — метод работает, и
    # поштучный обход стал бы 1300+ бессмысленных запросов к чужому API каждое
    # утро. Добор остаётся страховкой: отдаст выгрузка пусто — он включится сам.
    if any(str(row.get("serviceSynonyms") or "").strip() for row in rows):
        log.info("✅ синонимы пришли массовой выгрузкой, поштучный добор не нужен")
        return 0
    filled = 0
    for row in rows:
