...
async def vllm_call(prompt: str, *, max_tokens: int = 1024) -> str:
    """
    Асинхронный запрос к локальному vLLM HTTP-серверу.
    """
    payload = {
        "prompt": prompt,
    ...
    url = "http://192.168.1.42:8100/v1/completions"
    async with httpx.AsyncClient(timeout=30.0) as vllm_client:
        try:
            resp = await vllm_client.post(url, json=payload)
            resp.raise_for_status()
            data = resp.json()
            # Берём текст первого варианта
            return data["choices"][0]["text"]
        except Exception as e:
            logger.error(f"Ошибка при вызове vLLM: {e}")
            raise
...
async def formulate(llm_output: str) -> str:
    ...
    **Ничего не выдумывай!** Если врач не найден — просто верни это сообщение.
    ...
    aresult = await vllm_call(formatted_prompt, )
...
"""
    ...
    # Генерируем ответ
    aresult = await vllm_call(system_message, )
    print(f"\nОтвет LLM: {aresult}")
